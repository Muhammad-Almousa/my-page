				 ...
				 <tr>
			       <td>'.$num.'</td>
				    <td>'.substr($post['title'],0,200).'</td>
					 ...
					 <td>'.$post['username'].'</td>
					<td>'.$post['com_date'].'</td>
					<td><a href="../post.php?id='.$post['post_id'].'" class="btn btn-primary btn-xs" target="_blank">مشاهدة التعليق</a> </td>
				    <td>'.($post['status'] == 'dreft' ? '<a href="comment.php?status=published&post='.$post['com_id'].'&page='.$page.'" class="btn btn-success btn-xs ">نشر</a>' : '<a href="comment.php?status=dreft&post='.$post['com_id'].'&page='.$page.'" class="btn btn-info btn-xs">تعطيل</a>').'</td>	
				    <td><a href="edite_comment.php?post='.$post['com_id'].'" class="btn btn-warning btn-xs ">تعديل</a> </td>
