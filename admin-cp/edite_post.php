						 ...
						 if($image_error === 0){
							...
							$image_db = 'images/posts/'. $new_name;
									...
									$insert = mysqli_query($conn, "UPDATE `posts` SET `title` = '$title',`post` = '$post', `category` = '$category', `image` = '$image_db', `status` = '$status' WHERE `post_id` = '$_GET[post]'") ;
									 ...
									 if(isset($insert)){
												$msg = '<div class="alert alert-success" role="alert">تم تحديث المقال بنجاح.. جاري تحويلك إلى المقالات</div>';
	     ...
	     <div class="col-md-1"> </div>
			...
			?>
	          <div class="panel panel-info">
		       <div class="panel-heading"><b> تعديل المقال :</b>  <?php echo $post['title'];  ?></div>
		        <div class="panel-body">
				  <form action="" method="post" class="form-horizontal" enctype="multipart/form-data">
				  <div class="form-group">
					<label for="title" class="col-sm-2 control-label"> عنوان المقال</label>
					<div class="col-sm-4">
					  <input type="text" class="form-control" name="title" value="<?php echo $post['title'];  ?>" id="title" placeholder="أدخل عنوان المقال">
					</div>
				  </div>
