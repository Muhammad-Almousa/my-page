...
<article class="col-md-9 col-lg-9 art_bg">
      ...
      <div class="carousel-inner">
       <?php 
        ...
        <div class="item <?php echo ($x == 0 ? 'active' : ''); ?> ">
          <img src="<?php echo ($slide['image'] == '' ? 'images/No_image.png' : $slide['image']);  ?>" width="100%" style="height: 350px;">
           <div class="carousel-caption" style="position: absolute; bottom: 0px; z-index: 10; left: 0px; padding: 10px; color: #fff; text-shadow: 0 1px 2px rgba(0,0,0,.6); right: -10px; text-align: justify; background: #9d9d9d;" >
            <h3 style=" margin: 0px; position: absolute; top: -280px; width: 100%; background: #F1F1F1; padding: 8px; color: #7B7B7B; "><a href="post.php?id=<?php echo $slide['post_id'];  ?>"><?php echo $slide['title'];  ?></a></h3>
		...
		<?php 
		   ...
		   for($i = 0; $i<$count_slide; $i++){
		       echo '<li data-target="#myCarousel" data-slide-to="'.$i.'" '.($i == 0 ? 'class="active"' : '').'><a href="#"><i class="fa fa-star fa-fx" aria-hidden="true"></i></a></li>';	
		  }
		?>
        </ul>
	...
	<!-- category A -->
	<div class="row"><h2 class="tit_cat1"><?php echo $setting['section_a'];?></h2>
	<?php
        ...
        <div class="col-sm-4 col-md-4" style="margin-bottom: 20px">
            <div class="post">
                <div class="post-img-content">
                    <img src="<?php echo ($sec['image'] == '' ? 'images/No_image.png' : $sec['image']);  ?>" class="img-responsive" style="width: 100%;height: 200px;"/>
                ...
                <div class="content">
                    ...
                    <div class="text-justify">
                       <?php echo strip_tags(substr($sec['post'],0,300));  ?> ......
                    </div>
					<hr />
                    <div class="text-left">
		...
		<!-- tab -->
						...
						<div class="tab-content">
							...
							?>
								<div class="bg_tab_topic">
									<div class="col-md-3">
											<img src="<?php echo ($tab_a['image'] == '' ? 'images/No_image.png' : $tab_a['image'] ); ?>" width="100%" class="img-thumbnail" />
										</div>
										<div class="col-md-9">
										...
										<p class="col-md-12 text-justify">
											<?php echo strip_tags(substr($tab_a['post'],0,300)); ?>
										</p>
										</div>
							...
							<?php 
							 $tab_2 = mysqli_query($conn, "SELECT * FROM `posts` WHERE `status` = 'published' AND `category` = '$setting[tab_b]' ORDER BY `post_id` DESC LIMIT $setting[tab_b_value]");
							 while($tab_b = mysqli_fetch_assoc($tab_2)){
							?>
								<div class="bg_tab_topic">
									<div class="col-md-3">
											<img src="<?php echo ($tab_b['image'] == '' ? 'images/No_image.png' : $tab_b['image'] ); ?>" width="100%" class="img-thumbnail" />
										</div>
										<div class="col-md-9">
										<h3 class="col-md-12 text-justify" style="margin-top: 8px;background: #009688;padding: 8px;">
											<a href="post.php?id=<?php echo $tab_b['post_id']; ?>" class="a_1"><?php echo $tab_b['title']; ?> </a>
										</h3>
										<p class="col-md-12 text-justify">
											<?php echo strip_tags(substr($tab_b['post'],0,300)); ?>
										</p>
										</div>
									<div class="clearfix"></div>
								</div>
							...
							<?php 
							 $tab_3 = mysqli_query($conn, "SELECT * FROM `posts` WHERE `status` = 'published' AND `category` = '$setting[tab_c]' ORDER BY `post_id` DESC LIMIT $setting[tab_c_value]");
							 while($tab_c = mysqli_fetch_assoc($tab_3)){
							?>
								<div class="bg_tab_topic">
									<div class="col-md-3">
											<img src="<?php echo ($tab_c['image'] == '' ? 'images/No_image.png' : $tab_c['image'] ); ?>" width="100%" class="img-thumbnail" />
										</div>
										<div class="col-md-9">
										<h3 class="col-md-12 text-justify" style="margin-top: 8px;background: #009688;padding: 8px;">
											<a href="post.php?id=<?php echo $tab_c['post_id']; ?>" class="a_1"><?php echo $tab_c['title']; ?> </a>
										</h3>
										<p class="col-md-12 text-justify">
											<?php echo strip_tags(substr($tab_c['post'],0,300)); ?>
										</p>
										</div>
									<div class="clearfix"></div>
								</div>
			...
			?>
				<div class="bg_tab_topic col-md-6">
					<div class="col-md-4">
						<img src="<?php echo ($sec2['image'] == '' ? 'images/No_image.png' : $sec2['image'] ); ?>" width="100%" class="circle" />
					</div>
					<div class="col-md-8">
